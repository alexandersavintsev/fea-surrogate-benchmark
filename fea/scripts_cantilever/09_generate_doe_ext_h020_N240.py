...
DATA = os.path.abspath(os.path.join(HERE, "..", "data", "ext_h020_N240"))
...
def main():
    ...
    inp = os.path.join(DATA, "cantilever_doe_inputs_ext_h020_N240.csv")
    ...
    worst_u = max(beam(*c)[1] for c in [(14, 0.20, 0.02, 1800)])
    worst_s = max(beam(*c)[2] for c in [(14, 0.20, 0.02, 1800)])
    chk = os.path.join(DATA, "cantilever_theory_check_ext_h020_N240.csv")
