...
Цель - отдельно проверить влияние формы отклика. Контроль показал, что одного
порога недостаточно для смены лидера, а разрыв и резонансный пик дают
преимущество деревьям над GPR. Высокочастотная функция трудна для всех моделей.
...
DATASETS = [
    ...
    "threshold_relu",       # порог по гиперплоскости: GPR сохраняет лидерство
    "step_discontinuous",   # разрыв                                       -> деревья
    "high_frequency",       # высокочастотная осцилляция: низкое качество у всех
