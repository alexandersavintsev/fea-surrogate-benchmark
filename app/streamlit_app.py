...
if mode == "Готовые результаты полного эксперимента":
    ...
    st.info(
        "1000 расчётов распределены между пятью физическими постановками "
        "и не объединяются в один датасет. Для живого демо пересчитывается "
        "одна постановка: выбранные модели, 5 фолдов и 1 random seed. "
        "Полный эксперимент использует 8 моделей и 3 seed."
    )
    ...
    if st.button("Запустить быстрый пересчёт"):
